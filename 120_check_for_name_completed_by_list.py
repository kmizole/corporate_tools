...
def run (**kwargs):
  for i in [ 'domain', 'adapter', 'words_list', 'combine', 'combine_times', 'use_cache', 'cache_file' ]:
    if i not in kwargs:
  ...
  try:
    ...
    for a in shuffle_words_from_lists (domain, kwargs['words_list'], kwargs['combine'], kwargs['combine_times']):
      for d in compute_all_domains_tld (element = a, use_cache = kwargs['use_cache'], cache_file = kwargs['cache_file']):
