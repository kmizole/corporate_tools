...
def run (**kwargs):
  ...
  try:
    ...
    for d in compute_all_domains_tld (element = kwargs['name'], use_cache = kwargs['use_cache'], cache_file = kwargs['cache_file']):
...
if __name__ == "__main__":
  ...
  parser.add_argument ('--use-cache', required = False, default = True, \
    help = "Désactiver ce booléen pour ne pas utiliser le dossier de cache.", \
    action = 'store_false' )
  ...
  try:
    for current_name in args.name:
      ...
      for l in run (name = current_name, adapter = args.adapter, use_cache = args.use_cache, \
